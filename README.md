...
사용자의 음성 발화를 텍스트로 변환한 뒤, 답변 생성과 언어 특징 분석을 분리 수행하고, 결과를 대시보드 형태로 시각화하여 보여줍니다.

---

## 1. 프로젝트 개요

본 프로젝트는 일상 대화에서 나타나는 언어적 특징을 기반으로 인지 위험 신호를 보조적으로 관찰할 수 있는 웹 시스템을 구현하는 것을 목표로 합니다.

기존의 인지 관련 평가는 병원 방문 이후에 이뤄지거나 별도의 검사 상황에 의존하는 경우가 많습니다. 본 시스템은 사용자의 음성을 직접 입력받아 질문 반복, 기억 혼란, 시간·상황 혼란, 문장 비논리성과 같은 특징을 역할별로 분석하고, 그 결과를 직관적인 UI로 제시합니다.
...
- 인지 변화는 질문 반복, 최근 정보 회상 실패, 일정 및 시간 혼동 등 언어적 징후로 먼저 드러날 수 있습니다.
- 분석 결과를 수치와 시각 요소로 함께 제공하면 상태 변화를 더 쉽게 이해할 수 있습니다.
- 단순 대화형 챗봇이 아니라, 기록과 관찰 기능이 결합된 모니터링 시스템 형태가 필요합니다.
...
- 질문 반복, 기억 혼란, 시간·상황 혼란, 문장 비논리성을 역할별로 분석한다.
...
세부 분석과 함께 읽을 수 있도록 간결한 정보 구조로 구성했습니다.
...
세션의 변화 흐름을 확인하고, 리포트 기능으로 전체 분석을 정리할 수 있도록 했습니다.
...
일반 언어 분석과 함께 회상 과정을 보조적으로 확인할 수 있도록 구성했습니다.
