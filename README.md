...
본 시스템의 화면은 `음성 입력 → 답변 생성 → 역할별 분석 → 결과 시각화 → 기억 회상 테스트`의 흐름이 한 화면 안에서 자연스럽게 이어지도록 구성하였다.  
사용자는 채팅 기반 인터페이스를 통해 발화를 입력하고, 우측 분석 패널과 좌측 요약 패널을 통해 결과를 즉시 확인할 수 있다.

| 메인 인터페이스                                                      | 음성 입력 상태                                                 |
| -------------------------------------------------------------------- | -------------------------------------------------------------- |
| ![메인 화면](docs/images/01-overview.png)                            | ![녹음 진행 화면](docs/images/02-recording-state.png)          |
| 대시보드, 채팅, 분석 패널이 동시에 보이는 기본 구조의 메인 화면이다. | 사용자의 음성을 수집하는 동안 상태 변화가 시각적으로 반영된다. |

| 분석 반영 단계                                                         | 최종 분석 결과                                                       |
| ---------------------------------------------------------------------- | -------------------------------------------------------------------- |
| ![분석 진행 화면](docs/images/04-analysis-progress.png)                | ![최종 분석 결과 화면](docs/images/05-final-dashboard.png)           |
| 답변이 먼저 표시된 뒤 역할별 분석 점수가 순차적으로 반영되는 장면이다. | 최종 점수, 위험도, 추세, 세부 분석 결과가 모두 반영된 완료 화면이다. |

| 기억 회상 테스트                                                      |
| --------------------------------------------------------------------- |
| ![Recall Memory Test 화면](docs/images/06-recall-test.png)            |
| 세션 흐름에 따라 Recall Memory Test가 보조 평가 형태로 함께 표시된다. |

### 9.2 핵심 인터페이스 카드

각 카드는 단순한 시각 요소가 아니라, `현재 상태 파악`, `분석 근거 확인`, `세션 흐름 추적`이라는 목적에 따라 역할을 분리해 설계하였다.
...
위험도 단계와 보조 문구를 함께 제시하여 사용자가 전체 상태를 한눈에 인지할 수 있도록 구성하였다.
...
기본 화면에서는 접은 상태로 유지하고 필요 시 확장되도록 설계하여 화면 밀도를 안정적으로 제어한다.
...
역할별 분석 칩을 함께 배치하여 현재 어떤 분석이 진행 중인지 직관적으로 확인할 수 있도록 하였다.
...
과거 대화를 클릭했을 때 해당 시점의 분석 내용을 다시 확인할 수 있도록 구성하였다.
...
주요 분석 카드와 함께 읽히도록 간결한 정보 구조를 유지한다.
...
언어 기반 위험도 분석과 함께 기억 회상 흐름을 자연스럽게 연결하는 인터페이스 역할을 수행한다.
